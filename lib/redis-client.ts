...
// Import cache stats
import { recordCacheHit, recordCacheMiss, recordCacheSet, recordCacheError } from "@/lib/cache-stats"
import { getFromMemoryCache, setMemoryCache, deleteFromMemoryCache } from "./memory-cache"
  ...
  if (!redisClient) {
    try {
      ...
      const redis = await import("redis").catch(() => {
        console.warn("[Redis] Redis client cannot be imported in this environment")
        isRedisAvailable = false
        return null
      })
 ...
 */
export async function setCache(key: string, value: any, ttl = DEFAULT_CACHE_TTL): Promise<boolean> {
  try {
    // Allow caching of empty arrays - they are valid states
    // Only skip null and undefined values
    if (value === null || value === undefined) {
      console.log(`[Redis] Skipping cache for null/undefined value for key: ${key}`)
      return false
    }
