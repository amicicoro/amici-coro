import { getEventFolderPath, uploadToCloudinary, listImagesInFolder } from "./cloudinary"

// Base implementation without caching
export async function _getEventPhotos(slug: string): Promise<{ url: string; pathname: string; contentType: string }[]> {
  try {
...
export async function getEventPhotos(slug: string): Promise<{ url: string; pathname: string; contentType: string }[]> {
  try {
    ...
    const CACHE_KEYS = { EVENT_PHOTOS: (slug: string) => `events:${slug}:photos` }
