...
// Helper to map legacy event objects to the new Event type
export function mapLegacyEventToEvent(blobEvent: any, id?: string): Event {
  return {
    id: id || blobEvent.id || blobEvent.slug || '',
    slug: blobEvent.slug || '',
    title: blobEvent.title || '',
    subtitle: blobEvent.subtitle || '',
    startDate: blobEvent.startDate || blobEvent.date || '',
    endDate: blobEvent.endDate || blobEvent.end_date || '',
    venueId: blobEvent.venueId || blobEvent.venue_id || '',
    imageUrl: blobEvent.imageUrl || blobEvent.image_url || undefined,
    status: blobEvent.status || 'draft',
    description: blobEvent.description || '',
    schedule: Array.isArray(blobEvent.schedule)
      ? blobEvent.schedule.map((item: any) => ({
          date: item.date,
          startTime: item.startTime || item.time || '',
          description: item.description || '',
        }))
      : undefined,
    musicList: Array.isArray(blobEvent.musicList)
      ? blobEvent.musicList
      : undefined,
  };
}
...
async function _getEventById(id: string): Promise<Event | null> {
  try {
    ...
    try {
      ...
      return mapLegacyEventToEvent(eventData, id);
    } catch (parseError) {
...
export async function getEventById(id: string): Promise<Event | null> {
  try {
    ...
    const result = await cachedFn(id);
    return result ? mapLegacyEventToEvent(result, id) : null;
  } catch (error) {
    ...
    const events = await Promise.all(
      Array.from(eventBlobsMap.entries()).map(async ([eventId, eventBlobs]) => {
        try {
          ...
          try {
            const eventData = await response.json();
            return mapLegacyEventToEvent(eventData, eventId);
          } catch (parseError) {
            console.error(
              `Error parsing JSON for event ${eventId}:`,
              parseError,
            );
            throw parseError;
          }
...
export async function getAllEvents(): Promise<Event[]> {
  try {
    ...
    return Array.isArray(result)
      ? result.map((e: any) => mapLegacyEventToEvent(e, e.id))
      : [];
  } catch (error) {
...
export async function createEvent(event: Event): Promise<Event> {
  try {
    ...
    if (safeId !== event.id) {
      console.log(
        `Sanitized event ID from "${event.id}" to "${safeId}" for blob storage compatibility`,
      );
      event.id = safeId;
...
export async function updateEvent(event: Event): Promise<Event> {
  try {
    ...
    console.log(
      `Found ${eventBlobs.length} event files for event ID: ${event.id}`,
    );
    ...
    try {
      const result = await put(
        `data/events/${event.id}/${newFilename}`,
        eventJson,
        {
          contentType: 'application/json',
          access: 'public', // Add this line to make the blob publicly accessible
      ...
      console.log(
        `Successfully updated event ${event.id} to version ${newVersion}`,
      );
      ...
      console.error(
        `Error writing to blob storage for event ${event.id}:`,
        putError,
      );
      throw putError;
    }
...
// Base implementation without caching
async function _getEventPhotos(
  slug: string,
  ...
  try {
    ...
    const photos = photoBlobs.filter(
      (blob): blob is { url: string; pathname: string; contentType: string } =>
        blob !== null,
    );
...
async function _getEventBySlug(slug: string): Promise<Event | null> {
  try {
    ...
    return mapLegacyEventToEvent(event);
  } catch (error) {
...
export async function getEventBySlug(slug: string): Promise<Event | null> {
  try {
    ...
    // Call the cached function
    const result = await cachedFn(slug);
    return result ? mapLegacyEventToEvent(result) : null;
  } catch (error) {
...
export async function uploadEventPhoto(
  ...
  try {
    ...
    const blob = await put(
      `data/events/${slug}/photos/${uniqueFileName}`,
      file,
      {
        access: 'public',
