              ...
              <Card key={index}>
                <CardContent className="p-4">
                  <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2">
                    ...
                    <div className="text-muted-foreground">{format(new Date(item.date), "PPP p")}</div>
                  </div>
                </CardContent>
              </Card>
            ))}
              ...
              <div key={service}>
                ...
                <div className="grid gap-2">
                  {items.map((item, index) => (
                    <div key={index} className="flex flex-col md:flex-row md:items-baseline gap-1 md:gap-2">
                      {item.type && <span className="font-medium min-w-24">{item.type}:</span>}
                      <span>{item.title}</span>
                      {item.composer && <span className="text-muted-foreground">by {item.composer}</span>}
                    </div>
                  ))}
                </div>
              </div>
