  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden transition-transform duration-300 hover:scale-[1.02] flex flex-col h-full">
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <p className="text-sm text-gray-700 mb-4 flex-grow">{event.description}</p>
        <Link href={`/${isPastEvent ? "past-events" : "events"}/${event.slug}`} className="mt-auto">
          <Button className="w-full">{isPastEvent ? "View Details" : "Learn More"}</Button>
        </Link>
      </div>
    </div>
  )
